...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./krn_mrp.db")
...
def setup(db: SessionLocal = Depends(get_db)):
    ...
    # seed nothing, GRN prices shown are static defaults
...
@app.post("/grn/new")
def grn_new_post(date: str = Form(...), supplier: str = Form(...), rm_type: str = Form(...),
                 qty: float = Form(...), price: float = Form(...), db: SessionLocal = Depends(get_db)):
    g = GRN(date=dt.date.fromisoformat(date), supplier=supplier, rm_type=rm_type, qty=qty, remaining_qty=qty, price=price)
    db.add(g); db.commit()
...
def melting_new(request: Request,
    ...
    # gather lines
    lines = []
    for t,q in [(rm_type_1,rm_qty_1),(rm_type_2,rm_qty_2),(rm_type_3,rm_qty_3),(rm_type_4,rm_qty_4)]:
        if t and q and q>0: lines.append((t,float(q)))
    if not lines:
    ...
    for t,q in lines:
        if available_stock(db,t) < q - 1e-6:
            db.rollback()
            return PlainTextResponse(f"Insufficient stock for {t}. Available {available_stock(db,t):.1f} kg", status_code=400)
    for t,q in lines:
    ...
    heat.theoretical = total_inputs * 0.97  # 3% loss
    db.commit()
...
def qa_heat_save(heat_id:int, request:Request,
    ...
    heat.qa_status = decision
    heat.qa_remarks = remarks
    db.add(chem); db.add(heat); db.commit()
    return RedirectResponse("/melting", status_code=303)
...
def qa_lot_form(lot_id:int, request:Request, db: SessionLocal = Depends(get_db)):
    ...
    psd_map = {"+212":psd.p212 or "", "+180":psd.p180 or "", "-180+150":psd.n180p150 or "",
...
def qa_lot_save(lot_id:int,
    ...
    psd = lot.psd or LotPSD(lot=lot)
    psd.p212 = psd_fields.get("+212",""); psd.p180 = psd_fields.get("+180","")
    psd.n180p150 = psd_fields.get("-180+150",""); psd.n150p75 = psd_fields.get("-150+75","")
    psd.n75p45 = psd_fields.get("-75+45",""); psd.n45 = psd_fields.get("-45","")
    lot.qa_status = psd_fields.get("decision","APPROVED")
    lot.qa_remarks = psd_fields.get("remarks","")
...
def trace_lot(lot_id:int, request:Request, db: SessionLocal = Depends(get_db)):
    ...
    for h in heats:
        for cons in h.rm_consumptions:
            rows.append(type("Row", (), {"heat_no":h.heat_no,"rm_type":cons.rm_type,"grn_id":cons.grn_id,
                                         "supplier": cons.grn.supplier if cons.grn else "", "qty":cons.qty}))
...
def pdf_lot(lot_id:int, db: SessionLocal = Depends(get_db)):
    ...
    width, height = A4
    draw_header(c, "Traceability Report – Lot {}".format(lot.lot_no))
    ...
    for lh in lot.heats:
        ...
        y -= 12
        if y < 3*cm: c.showPage(); draw_header(c,"Traceability Report – Lot {}".format(lot.lot_no)); y = height-4*cm
    ...
    c.showPage(); c.save()
    buf.seek(0)
    return StreamingResponse(buf, media_type="application/pdf", headers={"Content-Disposition": f'inline; filename="trace_{lot.lot_no}.pdf"'})
