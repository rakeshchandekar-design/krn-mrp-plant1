...
def opening_stock_upload_page(request: Request):
    if current_role(request) != "admin":
        return RedirectResponse("/login", status_code=303)
    return templates.TemplateResponse(
        "opening_stock_upload.html",
        {"request": request, "user": current_username(request), "role": current_role(request)}
    )

@app.post("/opening-stock/upload")
async def opening_stock_upload(request: Request, file: UploadFile = File(...)):
    if current_role(request) != "admin":
        return RedirectResponse("/login", status_code=303)
    ...
    def _next_like(conn, table: str, prefix: str, column: str = "lot_no") -> str:
        last = conn.execute(text(f"SELECT {column} FROM {table} WHERE {column} LIKE :pfx ORDER BY {column} DESC LIMIT 1"), {"pfx": f"{prefix}%"}).scalar()
        seq = int(str(last).split("-")[-1]) + 1 if last else 1
        return f"{prefix}{seq:03d}"

    def _next_grn_no_conn(conn, on_date: dt.date) -> str:
        ymd = on_date.strftime("%Y%m%d")
        count = (conn.execute(text("SELECT COUNT(*) FROM grn WHERE grn_no LIKE :pfx"), {"pfx": f"GRN-{ymd}-%"}).scalar() or 0) + 1
        return f"GRN-{ymd}-{count:03d}"

    def _grade_to_family(fg_grade: str) -> str:
        g = _safe_str(fg_grade)
        if g.startswith("KFS "):
            return "KFS"
        if g.startswith("KIP M "):
            return "KIPM"
        if g.startswith("KSP "):
            return "KSP"
        if g.startswith("Oversize"):
            return "OVERSIZE"
        if g.startswith("Premix"):
            return "PREMIX"
        return "KIP"
    ...
    try:
        ...
        return _alert_redirect(f"Invalid Excel file: {e}", "/opening-stock/upload")

    expected = ["RM_OPENING", "PULV_OPENING", "ANNEAL_OPENING", "GRINDING_OPENING", "FG_OPENING"]
    missing = [s for s in expected if s not in wb.sheetnames]
    if missing:
        return _alert_redirect("Missing sheet(s): " + ", ".join(missing), "/opening-stock/upload")

    counts = defaultdict(int)

    with engine.begin() as conn:
        # RM opening -> real usable GRN stock
        ws = wb["RM_OPENING"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            rm_type, qty, unit_cost, qa_status, remarks = (row + (None, None, None, None, None))[:5]
            qty = _safe_float(qty)
            ...
            if unit_cost is None:
                unit_cost = 0.0
            qa_status = (_safe_str(qa_status) or "APPROVED").upper()
            remarks = _safe_str(remarks)
            supplier = "OPENING"
            # Per user note: Motherson job-work opening scrap is entered as Others at zero cost
            if _safe_str(rm_type).lower() == "others" and abs(unit_cost) < 1e-9:
                supplier = "Motherson Opening"
                if not remarks:
                    remarks = "Motherson job-work opening stock"
            grn_no = _next_grn_no_conn(conn, today)
            conn.execute(text("""
                INSERT INTO grn (grn_no, date, supplier, rm_type, qty, remaining_qty, price, transporter, vehicle_no)
                VALUES (:grn_no, :date, :supplier, :rm_type, :qty, :remaining_qty, :price, NULL, NULL)
            """), {
                "grn_no": grn_no,
                "date": today,
                "supplier": supplier,
                "rm_type": rm_type,
                "qty": qty,
                "remaining_qty": qty if qa_status != "REJECTED" else 0.0,
                "price": unit_cost,
            })
            counts["RM"] += 1

        # Pulverization opening
        ws = wb["PULV_OPENING"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            _date, _lot_no, grade, input_qty, mag_output_qty, cost_per_kg, qa_status, non_mag_raw, remarks = (row + (None,)*9)[:9]
            input_qty = _safe_float(input_qty)
            mag_output_qty = _safe_float(mag_output_qty)
            cost_per_kg = _safe_float(cost_per_kg)
            non_mag_raw = _safe_float(non_mag_raw)
            qa_status = (_safe_str(qa_status) or "APPROVED").upper()
            remarks = _safe_str(remarks)
            if not mag_output_qty or mag_output_qty <= 0:
                continue
            # Column H in uploaded workbook may contain qty in hand, not percentage.
            non_mag_qty = 0.0
            if non_mag_raw is not None:
                if non_mag_raw > 100:
                    non_mag_qty = non_mag_raw
                else:
                    # If a true % is supplied later, convert against feed qty
                    base_feed = input_qty or mag_output_qty
                    non_mag_qty = base_feed * (non_mag_raw / 100.0)
            if input_qty is None:
                # derive a consistent input quantity from mag output and non-mag qty; dust is treated as part of opening assumption if not provided
                input_qty = mag_output_qty + non_mag_qty
            dust_loss_pct = 3.0
            dust_loss_qty = max(input_qty * dust_loss_pct / 100.0, 0.0)
            feed_qty = max(input_qty - dust_loss_qty, 0.0)
            # If non-mag is explicitly given as qty, recompute feed around physical opening quantities
            if non_mag_qty > 0 and mag_output_qty > 0 and input_qty is not None:
                feed_qty = mag_output_qty + non_mag_qty
                dust_loss_qty = max(input_qty - feed_qty, 0.0)
            non_mag_pct = (non_mag_qty / feed_qty * 100.0) if feed_qty > 0 else 0.0
            if cost_per_kg is None:
                cost_per_kg = 0.0
            prefix = "PULV-" + today.strftime("%Y%m%d") + "-"
            lot_no = _next_like(conn, "pulv_lots", prefix)
            trace_id = f"OPEN-PULV-{lot_no}"
            job_card_no = f"JC-PULV-{today.strftime('%Y%m%d')}-{lot_no.split('-')[-1]}"
            conn.execute(text("""
            ...
            """), {
                "date": today,
                "lot_no": lot_no,
                "src_grn_json": json.dumps({"OPENING": input_qty}),
                "grade": _safe_str(grade) or "KRSP",
                "input_qty_kg": input_qty,
                "input_cost_per_kg": max(cost_per_kg - 3.0, 0.0),
                "process_cost_per_kg": 3.0,
                "dust_loss_pct": dust_loss_pct,
                "dust_loss_qty": dust_loss_qty,
                "feed_qty_kg": feed_qty,
                "mag_output_qty_kg": mag_output_qty,
            ...
            # QA header and params
            pulv_lot_id = conn.execute(text("SELECT id FROM pulv_lots WHERE lot_no=:lot_no"), {"lot_no": lot_no}).scalar()
            conn.execute(text("""
                INSERT INTO pulv_qa (pulv_lot_id, decision, remarks, ad, non_mag_pct)
                VALUES (:id, :decision, :remarks, NULL, :non_mag_pct)
            """), {"id": pulv_lot_id, "decision": qa_status, "remarks": remarks or "Opening stock", "non_mag_pct": non_mag_pct})
            counts["PULV"] += 1

        # Anneal opening
        ws = wb["ANNEAL_OPENING"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            _date, _lot, grade, weight_kg, cost_per_kg, qa_status, remarks = (row + (None,)*7)[:7]
            ...
            if cost_per_kg is None:
                cost_per_kg = 0.0
            qa_status = (_safe_str(qa_status) or "APPROVED").upper()
            prefix = "ANL-" + today.strftime("%Y%m%d") + "-"
            lot_no = _next_like(conn, "anneal_lots", prefix)
            trace_id = f"OPEN-ANL-{lot_no}"
            job_card_no = f"JC-ANL-{today.strftime('%Y%m%d')}-{lot_no.split('-')[-1]}"
            conn.execute(text("""
                INSERT INTO anneal_lots
                (lot_no, date, src_alloc_json, grade, weight_kg, rap_cost_per_kg, cost_per_kg, ammonia_kg, qa_status, remarks, created_by, trace_id, job_card_no)
                VALUES
                (:lot_no, :date, :src_alloc_json, :grade, :weight_kg, :rap_cost_per_kg, :cost_per_kg, 0, :qa_status, :remarks, :created_by, :trace_id, :job_card_no)
            """), {
                "lot_no": lot_no, "date": today, "src_alloc_json": json.dumps({"OPENING": weight_kg}),
                "grade": grade, "weight_kg": weight_kg, "rap_cost_per_kg": cost_per_kg,
                "cost_per_kg": cost_per_kg, "qa_status": qa_status, "remarks": _safe_str(remarks) or "Opening stock",
                "created_by": current_username(request) or "admin", "trace_id": trace_id, "job_card_no": job_card_no
            })
            counts["ANNEAL"] += 1

        # Grinding opening
        ws = wb["GRINDING_OPENING"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            _date, _lot, grade, weight_kg, cost_per_kg, qa_status, ov80, ov40, remarks = (row + (None,)*9)[:9]
            weight_kg = _safe_float(weight_kg)
            cost_per_kg = _safe_float(cost_per_kg)
            ov80 = _safe_float(ov80) or 0.0
            ov40 = _safe_float(ov40) or 0.0
            if not weight_kg or weight_kg <= 0:
                continue
            if cost_per_kg is None:
                cost_per_kg = 0.0
            qa_status = (_safe_str(qa_status) or "APPROVED").upper()
            prefix = "GRD-" + today.strftime("%Y%m%d") + "-"
            lot_no = _next_like(conn, "grinding_lots", prefix)
            conn.execute(text("""
                INSERT INTO grinding_lots
                (lot_no, date, anneal_lot_id, src_alloc_json, grade, weight_kg, oversize_p80_kg, oversize_p40_kg,
                 input_cost_per_kg, process_cost_per_kg, cost_per_kg, qa_status, remarks, created_by, trace_id, job_card_no)
                VALUES
                (:lot_no, :date, NULL, :src_alloc_json, :grade, :weight_kg, :ov80, :ov40,
                 :input_cost_per_kg, 0, :cost_per_kg, :qa_status, :remarks, :created_by, :trace_id, :job_card_no)
            """), {
                "lot_no": lot_no, "date": today, "src_alloc_json": json.dumps({"OPENING": weight_kg}),
                "grade": grade, "weight_kg": weight_kg, "ov80": ov80, "ov40": ov40,
                "input_cost_per_kg": cost_per_kg, "cost_per_kg": cost_per_kg, "qa_status": qa_status,
                "remarks": _safe_str(remarks) or "Opening stock", "created_by": current_username(request) or "admin",
                "trace_id": f"OPEN-GRD-{lot_no}", "job_card_no": f"JC-GRD-{today.strftime('%Y%m%d')}-{lot_no.split('-')[-1]}"
            })
            counts["GRIND"] += 1
        ...
        ws = wb["FG_OPENING"]
        for row in ws.iter_rows(min_row=2, values_only=True):
            _date, _lot, family, fg_grade, weight_kg, cost_per_kg, qa_status, remarks = (row + (None,)*8)[:8]
            weight_kg = _safe_float(weight_kg)
            cost_per_kg = _safe_float(cost_per_kg)
            if not weight_kg or weight_kg <= 0:
                continue
            if cost_per_kg is None:
                cost_per_kg = 0.0
            fg_grade = _safe_str(fg_grade)
            family = _safe_str(family) or _grade_to_family(fg_grade)
            qa_status = (_safe_str(qa_status) or "APPROVED").upper()
            prefix = "FG-" + today.strftime("%Y%m%d") + "-"
            lot_no = _next_like(conn, "fg_lots", prefix)
            conn.execute(text("""
                INSERT INTO fg_lots
                (date, lot_no, family, fg_grade, weight_kg, base_cost_per_kg, surcharge_per_kg, cost_per_kg,
                 src_alloc_json, qa_status, remarks, created_by, trace_id, job_card_no)
                VALUES
                (:date, :lot_no, :family, :fg_grade, :weight_kg, :base_cost_per_kg, 0, :cost_per_kg,
                 :src_alloc_json, :qa_status, :remarks, :created_by, :trace_id, :job_card_no)
            """), {
                "date": today, "lot_no": lot_no, "family": family, "fg_grade": fg_grade, "weight_kg": weight_kg,
                "base_cost_per_kg": cost_per_kg, "cost_per_kg": cost_per_kg,
                "src_alloc_json": json.dumps({"OPENING": weight_kg}), "qa_status": qa_status,
                "remarks": _safe_str(remarks) or "Opening stock", "created_by": current_username(request) or "admin",
                "trace_id": f"OPEN-FG-{lot_no}", "job_card_no": f"JC-FG-{today.strftime('%Y%m%d')}-{lot_no.split('-')[-1]}"
            })
            counts["FG"] += 1

    msg = "Opening stock uploaded successfully: " + ", ".join([f"{k}={v}" for k, v in counts.items() if v > 0])
    return _alert_redirect(msg or "No rows uploaded.", "/")
...
def kpi_atom_oversize(db: Session, start: dt.date, end: dt.date, yest: dt.date) -> tuple[float, float]:
    """
    Oversize = (PSD-derived >180/212 µm %) * lot weight + screening oversize.

    Fresh-database safe:
    - returns 0 if required tables are missing
    - only uses date filters when a date-like column actually exists
    """
    ...
    def date_expr(cols: set[str], alias: str = "") -> str | None:
    ...
    y_psd = m_psd = y_scr = m_scr = 0.0

    if table_exists("lot") and table_exists("lot_psd"):
        lot_cols = columns("lot")
        lp_cols = columns("lot_psd")
        if "weight" in lot_cols and ("p212" in lp_cols or "p180" in lp_cols):
            lot_date = date_expr(lot_cols, "l")
            lot_where_y = f" AND {lot_date} = :d" if lot_date else ""
            lot_where_m = f" AND {lot_date} BETWEEN :a AND :b" if lot_date else ""

            p212_expr = num('lp.p212') if 'p212' in lp_cols else 'NULL'
            p180_expr = num('lp.p180') if 'p180' in lp_cols else 'NULL'
            ...
            y_psd = run(sql_psd_y, d=yest) if lot_date else run(sql_psd_y)
            m_psd = run(sql_psd_m, a=start, b=end) if lot_date else run(sql_psd_m)

    if table_exists("screen_lot"):
        screen_cols = columns("screen_lot")
        if "oversize_80" in screen_cols:
            screen_date = date_expr(screen_cols)
            ...
            y_scr = run(sql_scr_y, d=yest) if screen_date else run(sql_scr_y)
            m_scr = run(sql_scr_m, a=start, b=end) if screen_date else run(sql_scr_m)
